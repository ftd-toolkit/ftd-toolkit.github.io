...
function healFull() {
  ...
  while (trauma > 0) {
    ...
    if (inputs["hd-medknow"] > 0 && inputs["hd-mednin"] > 0) {
      ...
      medhealed = Math.floor(mednin * 2/severity);
      total_med += medhealed;
      trauma -= medhealed;
    } else if (inputs["hd-mednin"] > 0) {
      // Do partial healing with medknow
      ...
      medhealed = Math.floor(medknow/severity);
      total_med += medhealed;
      trauma -= medhealed;
    }
    total_healed += healed;
  ...
  CUR_DAY += 1;
  days = "Days " + CUR_DAY + "-" + (CUR_DAY + days_needed - 1);
  CUR_DAY += (days_needed - 1);
